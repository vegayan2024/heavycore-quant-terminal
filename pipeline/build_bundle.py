...
"""
修复模块声明顺序与打包：
1. 先声明具体股票数据集 (sanyou, yihua, zhongwu, industryCommodityRegistry)
2. 再声明总标的库 masterUniverse (引用上述数据集)
3. 声明计算引擎 (张新民、交易系统、十大选股原则、估值带通道、跨品种叠加制图)
4. 声明 Canvas 高性能图表库
5. 声明各 UI 组件 (打分卡、仓位管理、财报面板、研报、标的库、选股器、制图弹窗、数据管理)
6. 声明并执行 AppController
"""
import os
import re
import sys
...
root_dir = r"c:\ai\antigravity"
terminal_dir = os.path.join(root_dir, "heavycore-quant-terminal")

for current_base in [root_dir, terminal_dir]:
    js_dir = os.path.join(current_base, "js")
    ...
    files_in_order = [
        ...
        os.path.join(js_dir, "data", "leaderDatasets.js"),
        os.path.join(js_dir, "data", "industryCommodityRegistry.js"),
        ...
        os.path.join(js_dir, "engine", "screenerTenPrinciplesEngine.js"),
        os.path.join(js_dir, "engine", "valuationBandEngine.js"),
        os.path.join(js_dir, "engine", "chartStudioEngine.js"),
        ...
        os.path.join(js_dir, "components", "screenerView.js"),
        os.path.join(js_dir, "components", "chartStudioModal.js"),
